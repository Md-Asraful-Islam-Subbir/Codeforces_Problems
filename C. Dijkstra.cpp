...
void dijkstra(int source)
{
    ...
    while (s.size() > 0)
    {
        ...
        {
            if (distan[child.first] > curdist + child.second)
            {
                v[child.first]=curv;
                distan[child.first] = distan[curv] + child.second;
                s.insert({distan[child.first], child.first});
            }
        }
    }
}
