...
int number;
    ...
    memset(answer, 0,sizeof answer);  //not -1 cz 0 initia
    for(auto u : mppp){
        int count = 1;
        for(int i = 1; i < u.second.size(); i++){
            if((u.second[i] - u.second[i - 1] - 1) & 1) continue;
            count++;
        }
        answer[u.first] = count;
    }
    for(int i = 1; i <= number; i++) cout << answer[i] << " ";
    cout << "\n";
  /*6
7
1 2 3 1 2 3 1
