...
ll n;
	...
	if (numodd == 0) {
		for (int i = 2; i < numeven - 1; i += 2) {
			cout << even[i] << " " << even[i + 1] << endl;
		}
		return;
	}
	if (numodd &1) {
		//numeven is also 1 mod 2
		for (int i = 1; i < numodd-1; i += 2) {
			cout << odd[i] << " " << odd[i + 1] << endl;
		}
		for (int i = 1; i < numeven-1; i += 2) {
			cout << even[i] << " " << even[i + 1] << endl;
		}
	}
	else {
