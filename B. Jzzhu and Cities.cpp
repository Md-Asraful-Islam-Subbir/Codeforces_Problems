...
vector<long long> d(N, 1e18), vis(N);

void dijkstra(int i){
    priority_queue<pair<pair<long long, bool>, int>, vector<pair<pair<long long, bool>, int>>, greater<pair<pair<long long, bool>, int>>> pq;
    pq.push({{d[i] = 0, 0}, i});
     for(int l = 0; l < k; ++l){
        int a, b;
        cin >> a >> b;
    ...
    while(!pq.empty()){
        long long l = pq.top().first.first, r = pq.top().first.second, x = pq.top().second;
        pq.pop();
        if(vis[x])
            continue;
        if(r)
