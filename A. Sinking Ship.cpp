...
void solution(){
 vector<pair<string,string>> people;
    ...
    while(n--){
        cin>>name>>type;
        people.emplace_back(name,type);
    }

    for(auto & item : people)
        if(item.second=="rat") cout<<item.first<<endl;

    for(auto & item : people)
        if(item.second=="child" || item.second=="woman") cout<<item.first<<endl;

    for(auto & item : people)
        if(item.second=="man") cout<<item.first<<endl;

    for(auto & item : people)
        if(item.second=="captain") cout<<item.first<<endl;
}
int main()
{
  optimize();  //fraction(a);
